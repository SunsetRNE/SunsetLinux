...
| 0.3.28 | 44 | **修「免 root 版铺不起来」：内嵌离线包里的 zstd 在 Android 上解不开**（模块沿用 **1.0.52**）：真机现场 —— 引导页第 3 步报 `✗ [解压] base：zstd 解压失败：No field ARRAY_BYTE_BASE_OFFSET of type I in class Lsun/misc/Unsafe`。根因：App 侧唯一的 zstd 解码器是 **aircompressor**（纯 Java，选它是因为 zstd-jni 没有 Android ABI），而它的解码路径依赖 `sun.misc.Unsafe.ARRAY_BYTE_BASE_OFFSET` —— **Android 的 `sun.misc.Unsafe` 是裁剪版、没有这个字段**，真机必然抛 `NoSuchFieldError`；桌面 JVM 单测有完整 Unsafe，所以一路是绿的（又一个「测试环境 ≠ 真机」）。更隐蔽的是：项目**本来就有** zstd 能力探测（探测失败就选 `.gz` 产物），但它只覆盖**频道下载**；而**内嵌离线包的 transport 是打包时写死的**（`.zst` 优先），解压时不探测 ⇒ 装了 `-full-` 的人反而在「零下载」这条路上撞墙。修法：**内嵌包一律用 gzip**（App 侧 gzip 走 JDK 自带）——① `tools/offline-bundle/mk-bundle.mjs` 的层取件偏好改成 **`.gz` 优先**，且**只有 `.zst` 时明确拒绝**（不再静默产出真机解不开的包，这正是本次事故的教训）；② 两处 CI 取件（`ci.yml` / `offline-bundle.yml`）改成 `url_gz \|\| url` —— 按「**谁能被 App 解开**」选件，而不是按体积选。代价：内嵌包/APK 变大（gzip 比 zstd 大约 40–50%），换来「装完零下载」真的成立。回归：`tools/offline-bundle/selftest.mjs` **27 → 28 / 0**（新增负例：只有 `.zst` 的层必须被拒并提示需要 `.gz`；合成夹具同步改用 `.gz`）、App 单测 **305 / 0**、其余门禁全绿 |
| 0.3.38 | 54 | **修「免 root 版铺不起来」的两处拦路虎：App 直接 exec 私有目录里的 `linuxctl`（`error=13`）+ 两个 runtime 的命令行不一致（`update: 未知参数 --version`）**（模块沿用 **1.0.52**）：这两处都是**真机连着撞出来的**，前一版修完立刻露出后一处，所以合并成一版发。<br>**① `error=13, Permission denied`（0.3.28 之后的下一处）** —— 现场：「安装内嵌离线包」最后一步报 `✗ [linuxctl update] base 失败: 无法执行 /data/user/0/…/files/sunsetlinux/bin/linuxctl: Cannot run program "…": error=13, Permission denied`。根因是**判据本身**：`LinuxCtl` 用 `File.canExecute()` 决定「直接 exec `bin/linuxctl`」还是「退化成 `sh <path>`」，而那个文件是 App 自己从 assets 铺下去的、**铺完就带执行位** ⇒ 走直接 exec；但 Android 上**执行位为真 ≠ 这个 `execve` 会成功**（挂载选项 / 系统策略 / ROM 差异，只差一台机器就复现），`ProcessBuilder.start()` 直接 `EACCES`。修法：契约收紧成**私有目录里的脚本永远由 `sh` 跑** —— 新增 `DshPaths.hostCtlCommand()` / `hostCtlCommandLine()` 作唯一构造入口（固定 `/system/bin/sh <path>`，**故意不判** `canExecute()`），proot 模式三条调用链全走它（`prootCommand` / `ptySpec` / `terminalCommand`），两处**就绪判据**（`ProotRuntime.isReady` / `ProotSetup.inspect`）从「存在且可执行」改成「**存在**」——"就绪"的语义是"`sh` 念得动它"，不是"内核肯 exec 它"。<br>**② `update: 未知参数 --version`（修完 ① 当场露出来的下一处）** —— 免 root 版的层装到 `base` 就失败：App 的 `OfflineApplier` 对两个 edition 用**同一份命令构造**（`update <layer> <file> --version <ver>`），root 版接受 `--version <ver>` 与 `--version=<ver>`，而 **proot 版一个都不认**（`-*) fail 2 "update: 未知参数 $1"`）⇒ 同一个按钮换个 edition 必然失败。这是**契约不一致**，不是用户操作错。修法：`runtime/proot/linuxctl.sh` 的 `cmd_update` 接住两种写法（与 root 版逐字对齐），并明确"proot 侧不从参数记版本"——它的版本一直在 rootfs 元数据里（`/etc/sunsetlinux-base-version` / `-runtime-version` / dsh 的 `package.json`），`status` 直接读那里；root 版要 state.json 是因为它的层是**只读镜像**、没有可读的 rootfs。<br>**回归**：App 单测 **305 → 312 / 0**（新增 `HostCtlCommandTest` 7 条，含 2 条**源码级**断言：`canExecute()` 不许再出现在这三处判据、宿主脚本 shebang 必须仍是 `#!/system/bin/sh` —— 这类错误在 JVM 里永远跑不出来：桌面没有 noexec，直接 exec 一律成功）；`runtime/proot/selftest.sh` **23 → 25 / 0**（bash 与 mksh 各一遍；新增 2 条**子进程级**断言：`update … --version <ver>` 与 `--version=<ver>` 都必须被接住，判据是错误停在"层文件不存在"而不是"未知参数"——专门守"两个 runtime 命令行漂移"这件事，并做了**变异验证**：把 `--version` 分支删掉这两条立刻变红）；真机（OnePlus PJD110 / Android 16）覆盖安装后重跑免 root 引导 |<br>**③ `rootfs 不存在：…（先 provision）`（装完 ② 当场露出来的第三处）** —— 免 root 版的「铺环境」走到 `update base` 时失败，而**这是引导用户的第一次安装**：`cmd_update` 里那句 `exists "$ROOTFS" || fail 1 "rootfs 不存在：$ROOTFS（先 provision）"` 把 `runtime`/`dsh` 的前提安到了 `base` 上 —— 而 `update_base` 的语义本就是"解包到 staging → 原子换掉 rootfs"，rootfs 不存在时照样成立（没有旧的就不用备份，`mv staging → rootfs` 即引导出第一棵 rootfs）。于是形成死锁：引导要 `update base` 才有 rootfs，而 `update base` 要求先有 rootfs。修法两处：① 存在性判据只对 `runtime`/`dsh` 生效（`base` 放行）；② `update_base` 里那句无条件 `mv "$ROOTFS" "$prev"` 在首次引导时**没有旧 rootfs 可备份**，`mv` 直接报 "cannot stat …" 把整条 update 打死 —— 现在改成"有旧的才备份、没有就只做 `mv staging → rootfs`"，并保持"要么没有 rootfs，要么是完整的 rootfs"（失败时清 staging，绝不留下半棵）；有旧 rootfs 时行为一字不变。<br>**④ proot 启动器缺执行位：`exec …/ld-linux-aarch64.so.1: Permission denied`（同期排查发现，必挡下一步）** —— `proot-bundle-arm64.tar.gz` 里的模式是**对的**（`bin/proot` 0755、`lib/ld-linux-*.so.1` 0700），但 App 用的是 **Android 的 tar（toybox），它不还原文件模式**，解出来一律 0600/0700；而 `proot-launch.sh` 走的是 `exec "$loader" --library-path "$lib" "$bin"`，**`bin/proot` 它自己 chmod 了、loader 却漏了** —— 而 loader 才是那次 exec 的主体 ⇒ 免 root 模式必死在启动那一步。这个坑在"用 GNU tar 解包"的开发机上看不见（GNU tar 还原模式），只有真机复现；修法：新增 `ProotRuntime.normalizeExecBits()`，在解包后按 tar 里的语义还原执行位（`bin/`、`lib/` 下的文件 + 所有目录；README/license/manifest 这类数据文件不动），`OfflineApplier` 每次装 proot 部件后调用并把**实际补了哪些文件**写进安装日志，`ProotRuntime.ensure()` 也顺带修一遍（幂等）—— 老版本已经装在盘上的环境因此不用卸载重装。<br>**⑤ `{LOCK_FD}: inaccessible or not found` —— 互斥锁在 mksh 下根本没建立，`update` 必挂**（装完 ③④ 后当场露出来的第五处，也是这几处里**最隐蔽**的一个）：日志 118 行只有一句<br>**⑥ `dsh 层里找不到 package.json` —— `update dsh` 只认"模块目录"形态，而官方 dsh 层是 rootfs 覆盖层**（把 ③④⑤ 修完后，整条引导终于跑到**最后一个部件**才露出来）：base 与 runtime 都装好了，dsh 报<br>**⑦ `proot-launch.sh` 在真机上没有执行位 + `start.sh` 选错了调用方式**（用户发来的 doctor 输出直接抓到）：`proot_binary: proot 存在但无法执行 --version：…/sunsetlinux/proot/proot-launch.sh（设备上的 /data 目录可能被 noexec 挂载，或缺少动态库…）`。两处根因：① 上一版 `normalizeExecBits` 只给 `bin/`、`lib/` 下的文件补执行位，**根目录的 `*.sh` 启动器漏了** —— 而 `proot-bundle-arm64.tar.gz` 里 `proot-launch.sh` 是 0755、Android 的 tar（toybox）不还原模式 ⇒ 解出来没有执行位；② `start.sh` 的 `resolve_proot` 判的是"**宿主解释器在不在**"（`[[ ! -x "$interp" ]]`），而 Android 上 `/system/bin/sh` 必然可执行 ⇒ 总走 else「直接 exec 包装器本身」，撞上①就当场失败。修法：① `normalizeExecBits` 的补位范围加上"根目录 `*.sh`"；② `resolve_proot` 改成按"**它是不是脚本**"决定 —— 是脚本就用 `sh <path>` 跑（与 App 侧同一条契约：宿主私有目录里的脚本一律经 sh，对 noexec 挂载同样成立），并在日志里说明用了哪个解释器。回归：App 单测 **313 / 0**（`解包后补执行位` 那条加断言：`proot-launch.sh` 必须被补上，README/license/manifest 仍不许动）、`runtime/proot/selftest.sh` **28 / 0**（bash 与 mksh 双跑）。<br>⚠️ **仍未定论**：`update base` 在真机上"版本行之后约 37 秒无声失败"（`LOCK_TIMEOUT=30` 的形状，但日志里没有被截断之外的错误行）。本版给 `cmd_update` 加了 **5 个阶段检查点**（`[1/5] 取锁…` … `[5/5] 落盘完成`），下一次真机运行的日志栏就能直接指出死在哪一步 （这一步属于"把失败变成可读证据"，不是修复本身）。<br>**⑧ 决策变更：默认自启（免 root 跟 App、root 跟模块），并移除全部环境/DSH 启停按钮**（用户原话：「启动应用默认启动相应的虚拟环境，进一步的启动只是启动 DSH（移除不必要的 UI 渲染绘制，进一步更新流程）；Root 模块刷写后，重启之后默认启动环境」；追加：「移除所有环境相关的启动按钮，免 root 版本，点开应用即启动环境；Root 版本默认环境一直运行。DSH 彻底和虚拟环境解绑，所有相关启动脚本彻底拆分」）。本版落地**界面与 App 行为**这一半：<br>**① 移除全部启停入口**：主界面的「一键启动（环境 + DSH）」「仅启动环境」「停止环境」「启动 DSH」「停止 DSH」与 proot 版的「启动环境 / 停止环境」主按钮**全部删除**，改为一条**如实的状态说明**（未部署 → 指向「部署」；运行中 → 说明每次打开都会确保它在跑；未运行 → 说明 App 正在自动拉起、久不起来去「诊断」）。`HintTarget.START` 不再指向"去启动"（那个动作不存在了），统一落到「诊断」；终端页的 `onStartEnvOnly` 传 null。**没有任何"去启动"的按钮 = 不会再有"我到底该按哪个"**。<br>**② 自动启动成为默认值**：`Prefs.autoStartEnv` 默认 `false → true`；设置页那个开关换成一行**说明**（`InfoRow`）—— 它已经不是可关的选项，留一个能关的开关只会把"默认自启"重新变成"要手动按"。<br>**③ 两级自动启动**（"进一步的启动只是启动 DSH"）：`LauncherViewModel.autoStartIfNeeded()` 从"只在 STOPPED 时 start"扩成两级 —— 环境没跑 → `start()`；**环境在跑但 DSH 没起 → `dshStart()`**。环境与 DSH 的生命周期从此各管各的。<br>**④ 一处独立发现并修掉**：doctor 报 `proot_binary: proot 存在但无法执行 --version：…/proot-launch.sh` —— 上一版的 `normalizeExecBits` 只补 `bin/`、`lib/` 下的执行位，**漏了根目录的 `*.sh`**；而 `start.sh` 的 `resolve_proot` 判的是"宿主解释器在不在"（Android 上 `/system/bin/sh` 必然在）⇒ 总走"直接 exec 包装器"，撞上没有执行位的文件就失败。现在：补位范围加上根目录 `*.sh`；`resolve_proot` 改成**按"它是不是脚本"**决定用 `sh <path>`（与 App 侧同一条契约）。<br>**回归**：App 单测 **313 → 314 / 0**（`StartModeContractTest` 与 `EditionSeparationTest` 里描述旧按钮矩阵的断言**改写成新契约**而不是删掉：不许再出现任何启停按钮/`vm.start()`、状态说明必须在、默认值必须是 true、两级自启动必须都在）、`runtime/proot/selftest.sh` **28 / 0**。<br>⚠️ **本版只落地界面与 App 行为这一半**：runtime 侧"启动脚本彻底拆分"（`linuxctl start` 不再起 DSH、DSH 独立 `dsh start/stop`、proot 的 entry/supervise 拆成环境守护 + DSH 监督两条）是下一半 —— 它要动 proot 的会话保活方式，需要单独一轮真机验证，不在本版里混着发。<br>**⑨ 找到「update base 无声失败约 37 秒」的真身：陈旧锁没人认领**（这一条是用户第三次真机排障后才定位的）：现场形状是「日志停在 `update base：[1/5] 取锁…`，约 37 秒后失败，且 App 里**没有任何错误行**」，`LOCK_TIMEOUT=30` 的形状对得上但日志里看不到超时文案。根因在 mkdir 型锁的**陈旧判据**：一次被杀的 update 会留下 `run/.linuxctl.lockd`（内含已死进程的 pid），而原来的清理条件是「pid 存在且已死 → 清理；否则年龄 > **120 秒**才清理」——两个漏洞：① 清理要等满 120s，而 `LOCK_TIMEOUT` 只有 30s ⇒ **每一轮更新都先白等 30 秒然后超时失败**；② `kill -0` 会被 **PID 复用**骗过 ⇒ 那个锁目录可能永远不清（这也解释了为什么重装 App 之后照样失败：`files/` 目录被清掉才会消失，而覆盖安装不清）。<br>修法：**两种判据都收紧** —— pid 死了（含 pid 文件缺失/为空）立即认领；pid 活着也看年龄，超过 `2×LOCK_TIMEOUT+10s`（最少 30s）即按陈旧处理；并且**超时前把证据打到 stderr**（持锁目录、pid、年龄、排查命令）—— 下一次真机排障不该再看到"无声失败"。<br>**验证**（容器里以 **App 身份 uid 10045** 复现真机现场：预置一个 pid 已死的 `.linuxctl.lockd`）：修前卡在 `[1/5] 取锁…` 直到超时；修后**立即认领**并完整跑完 —— `update base` → `update runtime` → `update dsh` → `provision` **四步全 exit=0**，`rootfs/bin/sh`、`rootfs/usr/local/bin/node` 就位、`@deepseek-ai/dsh` = 0.1.6-alpha.2。回归：`runtime/proot/selftest.sh` **28 / 0**（bash 与 mksh 双跑）、App 单测 **314 / 0**。<br>**⑩ doctor 的 proot 探测也走 `sh` + 归因补一类**（收尾两处）：① `cmd_doctor` 里探测 `proot --version` 原来是**直接 exec 包装器**，与 App 侧踩的是同一个坑（私有目录脚本没有执行位 / ptrace 上下文里 proot 拒绝），真机上表现为 `proot_binary: proot 存在但无法执行 --version：…/proot-launch.sh`，把用户指向"检查 noexec 挂载"这条错路；现在同样**经 `sh` 调用**，并加 `</dev/null` 与 `timeout 20`（防挂）。② `Diagnoser.classify` 补一类关键词（`等待锁超时`/`持锁目录`/`正在操作`/`陈旧锁` → 运行环境），让新的锁超时文案能被归因 —— 这是 `DiagnoserTest` 那条"运行时文案不许落到未知"的断言要求的（它当场就把这条新文案判红了，属于该测试在正常工作）。回归：App 单测 **314 / 0**、`runtime/proot/selftest.sh` **28 / 0**（bash 与 mksh 双跑）。
`✗ [linuxctl update] dsh 失败：dsh 层里找不到 package.json：…/dsh-0.1.6-alpha.2.erofs`。根因：`update_dsh` 假定"层根就是 npm 包的根（含 package.json）"，而**当前官方 dsh 层的真实形态是 rootfs 覆盖** —— 实测 `fsck.erofs` 解出来是 `usr/local/lib/node_modules/@deepseek-ai/dsh/…` 与 `root/.dsh/profiles/web/…`；更坑的是它找 package.json 用的 `find -maxdepth 3` 会**误命中** `root/.dsh/profiles/web/node_modules/<pkg>/package.json`，于是拿着一个不相干的依赖目录当"模块根"去 mv，最后仍然报"找不到 package.json"。<br>修法：① 判定改成**按结构**（层根有 `package.json` 或 `package/package.json` = 模块目录，否则 = 覆盖层），不再用启发式；② 覆盖层走新增的 `copy_tree` 合并进 rootfs，**旧 DSH 目录仍备份成 `.prev`**（保留回滚路），失败时回滚；③ 顺带把 `update_runtime` 里那句 `cp -R --preserve=mode,timestamps,links --no-preserve=ownership` 也收进 `copy_tree` —— **Android 的 cp（toybox）不认 `--no-preserve`**，一旦报错整层合并就失败，现在不认就退到通用 `cp -R`（两条路都保留符号链接，runtime 层里 `node -> /opt/node/bin/node` 这类链接必须原样）。<br>**验证**（容器里用**真层**跑完整序列，mksh = 设备上的 sh）：base → runtime → dsh 三层 `exit=0`；装完核对：`@deepseek-ai/dsh/package.json` = 0.1.6-alpha.2、`/usr/local/bin/dsh` 软链就位、`root/.dsh/profiles/web/` 已铺、`linuxctl provision` 通过、`start.sh` 走到"已后台启动"。
`/data/user/0/…/files/sunsetlinux/bin/linuxctl[2168]: {LOCK_FD}: inaccessible or not found`。根因：`acquire_lock` 用的是 bash 的**命名 fd** 语法 `exec {LOCK_FD}>"$RUN_DIR/.linuxctl.lock"` —— Android 的 `/system/bin/sh`（mksh）把它当**字面命令**去执行，于是 `flock -w … "$LOCK_FD"`（空串）必然失败，`update` 连解包都进不去；而 `flock` 在 mksh 里**恰好存在**（`/system/bin/flock`），所以永远走这一支、下面 mkdir 型锁的兜底根本轮不到 —— 这就是它"真机必挂、开发机（bash）不挂"的原因。<br>修法：判据从"有没有 flock"改成"**这个 shell 能不能真的锁上**"——流程里当场探一次（`exec 9>…` 在 mksh 下不报错，报错的是 `flock 9`），不成自动落到 shell 无关的 mkdir 型锁，并 warn 一行说明走了哪条路；`release_lock` 对称改成关 9 号 fd。顺带统一超时口径：mkdir 型锁原来写死 `i > 600`（≈120 s）**不认 `LOCK_TIMEOUT`**，而兜底那条恰恰是真机唯一会走的路 ⇒ 用户遇到别人持锁要干等两分钟且界面无提示；现在两边都按 `LOCK_TIMEOUT` 秒算。<br>**回归**：`runtime/proot/selftest.sh` **25 → 28 / 0**（bash 与 mksh 各一遍；新增 3 条：①"别人持锁时第二个进程会等锁并超时（退出码 3）"—— 用**确定性布景**（手工造锁目录 + 一个活着的 pid），因为这是唯一能证明互斥**真的建立**的判据，而原来那个 bug 的表现正是"锁根本没建立"，开发机上完全看不见；②持锁者退出后锁可再次获取；③源码级：不许再出现 `exec {VAR}>`（**跳过注释行**——修法本身就写了注释解释这个坑，第一版直接把解释判红了）。另外这一条也做过**变异验证**：把 `--version` 分支删掉时两条断言同时红过。
