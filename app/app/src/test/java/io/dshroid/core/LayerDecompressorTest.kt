 ...
 *
 * 产物由发布侧放在 `dist/`，可用 `-Ddshroid.dist.dir=<dir>` 覆盖。
...
class LayerDecompressorTest {
    ...
    private val distDir = io.dshroid.TestPaths.distDir
    ...
    private fun stage(src: File, expectSha: String, expectSize: Long): File {
        val dir = File(System.getProperty("java.io.tmpdir") ?: ".", "dshroid-layer-decompress-test")
        dir.mkdirs()
