 ...
 *
 * 仍可用系统属性覆盖：
 *   `-Ddshroid.repo.dir=<仓库根>`、`-Ddshroid.dist.dir=<产物目录>`
 *
 * Gradle 下也可在 `build.gradle.kts` 里统一传：
 *   `tasks.withType<Test> { systemProperty("dshroid.repo.dir", rootProject.projectDir.absolutePath) }`
 */
object TestPaths {
    ...
    val repoRoot: File = run {
        val override = System.getProperty("dshroid.repo.dir")
        if (!override.isNullOrBlank()) {
            val f = File(override)
            require(f.isDirectory) { "-Ddshroid.repo.dir 指向的不是目录：$override" }
            f
        } else {
    ...
    /** 产物目录（默认 `<仓库根>/dist`）。 */
    val distDir: File =
        System.getProperty("dshroid.dist.dir")
