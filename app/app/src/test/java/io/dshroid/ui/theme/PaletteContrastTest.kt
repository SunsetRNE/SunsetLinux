...
class PaletteContrastTest {
    ...
    @Test
    fun `全仓 UI 颜色字面量都是灰阶`() {
        val uiDir = File(repoDir, "app/app/src/main/java/io/dshroid")
    ...
    fun `主题不允许再跟随系统、也不允许出现第二套色板`() {
        val themeFile = File(repoDir, "app/app/src/main/java/io/dshroid/ui/theme/Theme.kt")
    ...
    fun `所有 Activity 都走统一的系统栏入口`() {
        val dir = File(repoDir, "app/app/src/main/java/io/dshroid")
