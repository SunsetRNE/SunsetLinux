...
class DiagnoserTest {
    ...
    private val realWorldCases: List<Triple<String?, FailureKind, String>> = listOf(
        // —— 挂载类（v1 全部漏判，退化成"启动"）——
        Triple("无法创建挂载点 /data/sunsetlinux/rootfs/mnt/sdcard", FailureKind.MOUNT, "创建挂载点失败"),
        Triple("无法创建层挂载点 /data/sunsetlinux/layers-mnt/base", FailureKind.MOUNT, "创建层挂载点失败"),
        Triple("无法卸载残留的层挂载点 /data/sunsetlinux/layers-mnt/dsh", FailureKind.MOUNT, "残留挂载点卸不掉"),
        ...
        Triple("overlay 挂载失败（lowerdir/upperdir/workdir 见 /data/sunsetlinux/run/linux.log）", FailureKind.MOUNT, "overlay 挂载失败"),
        Triple("无法创建 overlay upper/work 目录", FailureKind.MOUNT, "overlay 目录建不出来"),
        Triple("无法创建 /data/sunsetlinux/rootfs", FailureKind.MOUNT, "rootfs 目录建不出来"),
        ...
        Triple("无法卸载残留的 /data/sunsetlinux/layers-mnt/runtime", FailureKind.MOUNT, "残留挂载点"),

        // —— 缺失类 ——
        Triple("缺少层文件 /data/sunsetlinux/layers/base.erofs（也没有 base.squashfs）", FailureKind.MISSING, "层文件缺失"),
        Triple("缺少可写层镜像 /data/sunsetlinux/upper.img", FailureKind.MISSING, "可写层镜像缺失"),
    ...
    fun `挂载类中文文案不再退化成笼统的启动失败`() {
        val v1Broken = listOf(
            "无法创建挂载点 /data/sunsetlinux/rootfs/proc",
            "无法创建层挂载点 /data/sunsetlinux/layers-mnt/base",
            "无法卸载残留的层挂载点 /data/sunsetlinux/layers-mnt/dsh",
    ...
    private fun expandShellVars(text: String): String =
        text.replace(Regex("""\$\{[A-Za-z_][A-Za-z0-9_]*\}"""), "/data/sunsetlinux/x")
            .replace(Regex("""\$[A-Za-z_][A-Za-z0-9_]*"""), "/data/sunsetlinux/x")
    ...
    fun `挂载失败时给出挂载归因与诊断建议`() {
        val status = DshStatus.unavailable("无法创建挂载点 /data/sunsetlinux/rootfs/proc").copy(state = EnvState.ERROR)
