...
fun WelcomeScreen(
    ...
    Column(
        ...
        Column(Modifier.padding(horizontal = 20.dp, vertical = 14.dp)) {
            Text("欢迎使用 DSHroid", style = MaterialTheme.typography.headlineSmall)
            Text(
...
private fun StepBranch(state: WelcomeState, onOpenProvision: () -> Unit) {
    if (state.mode == EnvMode.ROOT) {
        ...
        Spacer(Modifier.height(6.dp))
        Text(
            text = "模块负责把运行时铺到 /data/linux，并在开机时启动环境（这样环境就不依赖 App 是否存活）。",
            style = MaterialTheme.typography.bodySmall,
            color = TextSecondary,
        )
        Spacer(Modifier.height(14.dp))
        ...
        DshCard(Modifier.fillMaxWidth()) {
            Column(Modifier.fillMaxWidth()) {
                ...
                when (state.suAvailable) {
                    ...
                    false -> WarnBox(
                        "检测不到 su。请打开 KernelSU / Magisk 管理器，给「DSHroid」授予 root 权限；" +
        ...
        DshCard(Modifier.fillMaxWidth()) {
            Column(Modifier.fillMaxWidth()) {
                ...
                NumberedLine(2, "选择模块包：dist/dshroid-module-0.1.0.zip")
                NumberedLine(3, "安装完成后**重启手机**（模块在 late_start 阶段生效）")
                NumberedLine(4, "重启后回到这里，点下面的「重新检测」，再继续部署层")
                Spacer(Modifier.height(10.dp))
                Text(
                    text = "模块包由仓库的 module/mkmodule.sh 产出（dist/dshroid-module-<版本>.zip）。" +
                        "它不删除 /data/linux，卸载模块也会保留你的数据。",
                    style = MaterialTheme.typography.labelSmall,
                    color = TextMuted,
                )
            }
        ...
        Surface(
            ...
            Row(
                ...
                Column(Modifier.weight(1f)) {
                    ...
                    Text(
                        "勾选后才能进入下一步（层部署需要 /data/linux 已就绪）",
                        style = MaterialTheme.typography.labelSmall,
                        color = TextMuted,
                    )
                }
        ...
        DshCard(Modifier.fillMaxWidth()) {
            Column(Modifier.fillMaxWidth()) {
                ...
                Spacer(Modifier.height(8.dp))
                NumberedLine(1, "拿到运行时包：dist/dshroid-proot-runtime.tar.gz（或 proot-bundle-arm64.tar.gz）")
                NumberedLine(2, "解到 App 私有目录：files/linux/（即 \$APP_FILES/linux）")
                NumberedLine(3, "确保 files/linux/bin/linuxctl 存在且可执行")
                Spacer(Modifier.height(10.dp))
                Text(
