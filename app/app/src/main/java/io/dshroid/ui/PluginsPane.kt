...
import io.dshroid.core.DshPlugins
...
import io.dshroid.ui.theme.LineStrong
...
class PluginsPaneState internal constructor(
    ...
    private suspend fun envReady(): Pair<LinuxCtl, String?> {
        ...
        if (status.state != io.dshroid.core.EnvState.RUNNING) {
    ...
    private fun run(label: String, block: suspend (LinuxCtl) -> io.dshroid.core.CtlResult) {
        if (busy) return
        busy = true
        ...
        scope.launch {
            ...
            val result = try {
                ...
                io.dshroid.core.CtlResult.fail(t.message ?: "执行失败")
            }
            busy = false
            if (result.ok) {
