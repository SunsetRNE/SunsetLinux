...
object Diagnoser {
    ...
    fun classify(lastError: String?): FailureKind {
        ...
        // 互斥锁等待超时（真机 2026-09-19：陈旧锁让每次 update 都白等 30s 再失败）。
        // 归到"运行环境"：它不是缺文件也不是挂载问题，处置动作是"看是谁占着锁"，与 RUNTIME 的
        // 下一步（跑一键诊断 / 看日志）一致。**必须放在 MOUNT 之前判** —— 这条文案里有
        // 「操作」「进程」等词，落到后面会被挂载那组抢走。
        if (any(e, "等待锁超时", "持锁目录", "正在操作", "陈旧锁")) {
            return FailureKind.RUNTIME
        }
