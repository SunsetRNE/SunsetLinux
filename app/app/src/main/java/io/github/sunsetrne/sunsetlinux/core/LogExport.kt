 ...
 *
 * 分享走 FileProvider（`io.github.sunsetrne.sunsetlinux.files`），不需要任何存储权限。
 */
object LogExport {
    ...
    suspend fun buildBundle(
        ...
        val file = File(exportDir(context), "sunsetlinux-report-$stamp.txt")
    ...
    fun share(context: Context, file: File, subject: String = "SunsetLinux 排障包") {
        val uri = FileProvider.getUriForFile(context, "${context.packageName}.files", file)
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = "text/plain"
            putExtra(Intent.EXTRA_SUBJECT, subject)
            putExtra(Intent.EXTRA_STREAM, uri)
            putExtra(Intent.EXTRA_TEXT, "SunsetLinux 排障包：${file.name}")
