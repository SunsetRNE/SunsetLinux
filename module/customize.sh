...
if [ ! -f "$LINUX_HOME/layers/dsh.erofs" ] && [ ! -f "$LINUX_HOME/layers/dsh.squashfs" ]; then
  ...
  ui_print ""
  ui_print "   sh $MODDIR/bin/device-provision.sh --seeds $LINUX_HOME/seeds"
  ui_print ""
  ui_print " ★ 不需要 bash、也不需要 Termux：1.0.5 起 device-provision.sh 是 mksh 原生的，"
  ui_print "   设备自带的 sh 直接跑（旧版会在这一步要求 bash —— 而 Android 根本没带 bash）。"
  ui_print ""
