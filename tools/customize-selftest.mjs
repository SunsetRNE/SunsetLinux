...
// ---- 2) 已有层：不该再念 provision 那一大段 --------------------------------
{
  ...
  // 用户 2026-09-19 要求：**移除「是否需要挂载」这个提示**，改成陈述"实机挂载行为"。
  // 所以这两条要反过来钉住：既不能出现旧提示，也必须出现新陈述。
  if (!/是否需要挂载/.test(out)) ok(`${tag}：不再输出「是否需要挂载」（按用户要求移除）`);
  else bad(`${tag}：还在输出「是否需要挂载」—— 用户已要求移除`);
  if (/实机挂载行为/.test(out)) ok(`${tag}：改为陈述「实机挂载行为」`);
  else bad(`${tag}：没有"实机挂载行为"的说明（用户要的是这个）`);
