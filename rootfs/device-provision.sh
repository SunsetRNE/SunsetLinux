#!/usr/bin/env bash
# =============================================================================
# dshroid · rootfs/device-provision.sh
#
...
#
# 它做什么（每一步都可重复执行）：
#   1. 建 /data/linux/{layers,seeds,cache,etc,bin,run,snapshots,upper,work,rootfs,layers-mnt}
...
locate_assets() {
    local cand
    for base in "$SELF_DIR" "$SELF_DIR/.." "$SELF_DIR/../.." "$LH/bin" "/data/adb/modules/dshroid/bin"; do
        [ -d "$base" ] || continue
...
write_resolv_conf() {
    ...
    {
        printf '# generated by dshroid device-provision.sh\n'
...
# 由 dshroid device-provision.sh 生成（可重现的官方源）
Types: deb
URIs: $mirror
...
build_runtime() {
    ...
    # --- 运行时入口脚本：/opt/dshroid/{entry.sh,supervise.sh} -----------------
...
build_dsh() {
    ...
    # 把素材搬进 chroot（放 /opt/dshroid，与 entry.sh 同目录）
    in_chroot /bin/mkdir -p /opt/dshroid
    copy_into_chroot "$INSTALL_WEB_PROFILE" /opt/dshroid/install-web-profile.sh 0755
    in_chroot /bin/mkdir -p /opt/dshroid/web-profile
    local tf
    for tf in "$WEB_PROFILE_TEMPLATE"/*; do
        [ -f "$tf" ] || continue
        copy_into_chroot "$tf" "/opt/dshroid/web-profile/$(basename "$tf")" 0644
    done
    ...
    if ! in_chroot /bin/bash /opt/dshroid/install-web-profile.sh \
            /opt/dshroid/web-profile /root/.dsh/profiles/web; then
...
# /root/.dsh-defaults
#
# 这里是 DSHroid 分发的 DSH 默认配置模板，随只读 dsh 层（L2）发布。
...
make_upper() {
    ...
    if [ -x "$BUILD_DIR/sbin/mke2fs" ] || [ -x "$BUILD_DIR/usr/sbin/mke2fs" ]; then
        if in_chroot /sbin/mke2fs -F -q -t ext4 -m 0 -L dshroid-upper "/upper.img.tmp" 2>/dev/null; then
            ok=1
        fi
    fi
    if [ "$ok" != "1" ]; then
        ...
        for mk in /system/bin/mke2fs /system/bin/mkfs.ext4; do
            [ -x "$mk" ] || continue
            if "$mk" -F -q -t ext4 -m 0 -L dshroid-upper "$img" >>"$LOG_FILE" 2>&1; then ok=1; break; fi
        done
    fi
...
main() {
    ...
    local inner="$CACHE_DIR/.provision-inner.sh"
    {
        ...
        printf 'export UPPER_SIZE_MB=%q DSHROID_PORT=%q\n' "$UPPER_SIZE_MB" "${DSHROID_PORT:-3080}"
