...
#### ⑥ 宿主通道（0.3.21 / 模块 1.0.45）：把"已经存在的访问"变成有名字、有提示、有审计

| 事实 | 证据 |
|---|---|
| 环境进程本来就是宿主 **root** | `uid=0`；`/proc/<pid>/attr/current` = **`u:r:ksu:s0`** |
| chroot **不是**安全边界 | 环境里 `/proc` 是宿主 PID 视图（内核无 `CONFIG_PID_NS`）；`/proc/1/root/data/sunsetlinux` 实测可读 |
| ⇒ 缺的是可见性/约定/审计 | 不是权限。任何"提权"说法都要先纠正这一点 |

实现：环境内 `/opt/sunsetlinux/host-channel.sh status|run <cmd>`（`start.sh` 的启动器同步会把它带进 rootfs），
开关在**宿主侧** `$LINUX_HOME/etc/host-channel.json`（chroot 的 `/etc` 是环境自己的 ⇒ 环境内看不到也改不到），
以 `linuxctl host-channel on|off` 控制；**默认关**，未启用时 `run` 退 3 且不执行；
命令里的绝对路径 token 必须在 `allow` 前缀下（退 4，**自测当场抓出**按参数逐个检查等于没查的洞）；
每次调用（含被拒）写审计 `run/host-channel.log`。

定位（写进文档，别当边界用）：**防手滑 + 留痕**，不是沙箱 —— 变量/相对路径可绕过白名单，
...
2. ~~宿主 root 通道~~ ⇒ **已做（0.3.21 / 模块 1.0.45）**：见 §3.10.58 ⑥ 与 `docs/host-channel.md`。
   仍待做的是**升级成"宿主侧常驻 worker"**（环境写请求、`sunsetd` 执行并回写结果；不依赖 chroot 逃逸，天然可限流/可结构化白名单）。
