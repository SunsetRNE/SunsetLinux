...
点 App **顶栏的 DSH 图标**（2026-09-16 起 DSH 从底栏移到顶栏；底栏是 启动 / 插件 / 终端）。
**期望**：直接看到 DSH Web GUI（不是 401 纯文本）；按返回（手势或返回键）**回主界面**，不是退出 App。

若显示 401：说明 App 加载的不是带令牌地址。把首页状态卡的原始 status JSON 发我。

### 7.0 内置终端（底栏「终端」）

环境**已经在运行**时，切到底栏「终端」→ 点「连接」→ 敲 `uname -a`、`ls /`。

| 期望 | 说明 |
|---|---|
| 状态胶囊变「已连接」，出现 `❯ 你的命令` 回显与命令输出 | 回显是**本面板补的**：管道里的 shell 是非交互的，自己不打提示符也不回显 |
| 命令确实跑在环境里（`uname -a` 是 Linux、`ls /` 是 rootfs 的内容） | root 模式 = `nsenter … chroot` 进 rootfs；proot 模式 = proot 内的 bash |
| 切到别的 tab 再切回来，**会话还在**（滚动也还在） | 会话挂在外壳上，不随 tab 销毁 |
| 点「停止」或退出 App 后，会话结束、**不留孤儿 shell** | root 终端里 `ps -A` 不该多出常驻的 bash |

> ⚠️ **两个"看着像 bug 其实不是"的现象**（终端**没有 PTY**，只能行缓冲）：
> 1. `vim` / `htop` / `top` 这类**全屏程序不可用**；`Ctrl-C`、`fg`、作业控制也没有。
> 2. 输出**按行刷新**：像 `python3` 的 `>>> ` 这种不换行的提示不会及时出现。
