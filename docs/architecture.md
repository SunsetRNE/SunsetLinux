...
4. 状态卡显示 `mode` 徽章 + 语义色（绿=运行 / 蓝=过渡 / 黄=停止中 / 红=错误 / 灰=停止）。

**★ 配色 token（App 与模块 WebUI 共用一套，改这里就要两边同步）**：

| 用途 | 值 |
|---|---|
| 页面底 / 卡片 / 次级面 / 描边 | `#070C16` / `#0E1626` / `#152033` / `#243350` |
| 主色青（亮）/ 蓝 / 薄荷绿 | `#22D3EE`（`#67E8F9`）/ `#38BDF8` / `#34D399` |
| 状态：运行 / 过渡 / 停止中·警告 / 错误 / 停止 | `#34D399` / `#38BDF8` / `#FBBF24` / `#F87171` / `#64748B` |
| 正文 / 次要 / 弱化 | `#E8EFFA` / `#9DB0CB` / `#6B7C96` |
