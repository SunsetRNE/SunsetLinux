...
| 维度 | 做法 |
|---|---|
| **侧边栏** | `ModalNavigationDrawer`：顶栏汉堡 + 边缘右滑；设置类入口（更新 / 诊断 / 频道 / 日志导出 / 冻结与省电豁免 / 重新部署 / 恢复出厂 / 关于）全部收进侧边栏 |
| 顶栏 | 汉堡 + 标题/阶段 + **DSH 入口** + 模式胶囊 + 刷新。DSH 原来占底栏一格、标签被挤到换行（真机反馈），已上移 |
| 底栏 | **胶囊式导航栏 3 tab**：启动 / 插件 / **终端**。底栏只渲染 `ShellTab.inCapsule` 为真的页（有 `ShellLayoutContractTest` 守着，避免又被塞满） |
