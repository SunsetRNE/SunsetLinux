...
2. ~~宿主 root 通道的显式开关~~ ⇒ **已做**（0.3.21 / 模块 1.0.45，`docs/host-channel.md`）：默认关、开关在宿主侧、
   白名单 + 全量审计。README 级结论：**它不新增权限**（环境本来就 uid 0 + `u:r:ksu:s0`，`/proc/1/root` 通着），
   价值全在可见性/约定/审计；下一个可选升级是"宿主侧常驻 worker"（不依赖 chroot 逃逸）。
   ⚠️ 本项目自己的 AI 会话**不得**用它执行设备 shell 守卫拒绝过的命令。
3. **API key 别走命令行**（壳侧）。

---

# ★★ 收束（2026-09-19 03:xx）—— 落点、你要做什么、下一轮从哪开始

## 一句话落点

**App 0.3.21 / 37 + 模块 1.0.45** 已发出（run 63）：本轮把用户点名的四件事做完 ——
视角地图、交换目录、环境内默认全权、壳读取提速 —— 外加**宿主通道（默认关）**与**自述文件重写**
（去掉别的移植项目的上游叙事，改成对接真正的 DSH 上游 `@deepseek-ai/dsh`）。

## 恢复上下文按这个顺序读（3 分钟）

1. 本文 **第 55 轮**（三件事的根因）+ 本节。
2. `docs/STATUS.md` **§3.10.57**（上一轮：频道永远验签失败的根因 = 原生 Android 没有 Ed25519 的 KeyFactory）、
   **§3.10.58**（本轮：视角/权限/壳）。
3. **`docs/viewpoints.md`** —— 改代码前必读（三个命名空间、路径映射、权限模型、怎么验）。
...
**宿主通道**现在是「薄包装」版（环境内 `chroot /proc/1/root` 跳出 + 白名单 + 全量审计，**默认关**）。
它的定位是"防手滑 + 留痕"，**不是安全沙箱** —— 你自己也提到"路径不太对"，我的判断与你一致：
- 想**更正统**：改成「宿主侧常驻 worker」（环境写请求进 `run/host-queue/`，`sunsetd` 执行并回写结果），
  不依赖 chroot 逃逸，天然可限流、可按请求类型做结构化白名单。代价是一轮的量。
- 或者**直接删掉**这条通道（反正 `/proc/1/root` 那条路本来就在，只是没有名字与审计）。

## 下一轮从哪开始（按优先级）

1. **挂载回流的真隔离**：先按 `linuxctl doctor` §1f 的判据确认（环境跑着时
   `grep -c sunsetlinux/rootfs /proc/1/mountinfo`），再实现 propagation（`/` 是 shared 时
   toybox 的 `unshare --propagation` 不可用，要自己处理）。影响：两个环境同起会互踩挂载。
2. **宿主通道**：按上面的拍板 worker 化 / 删除 / 加 App UI 开关。
3. **API key 别走命令行**（壳侧，已提醒）：改成环境文件注入。
4. 老账：dsh 层里那 5 个重复入口脚本副本（会静默吞掉 runtime 层的改动），等下次 dsh 升级一起修。
